...
async function tryGroq(text: string, voice: string): Promise<Response | null> {
  ...
  try {
    const res = await fetch('https://api.groq.com/openai/v1/audio/speech', {
      ...
      body: JSON.stringify({
        model: 'playai-tts',
        input: text.slice(0, 10000), // Groq limit
...
async function tryEdgeTTS(text: string, lang: string): Promise<Response | null> {
  // Edge TTS uses a websocket-like protocol via REST.
  // We use the well-known free Edge TTS REST wrapper pattern.
  try {
    ...
    // Use the public Edge TTS REST API
    ...
    // Use the free edge-tts compatible REST endpoint
...
function buildFallbackWav(text: string): Response {
  ...
  // Build using ArrayBuffer + DataView for maximum compatibility
  ...
  // Generate a pleasant tone sequence (not just a flat beep)
...
export async function POST(req: Request) {
  try {
    ...
    // Emergency fallback — always return valid audio
